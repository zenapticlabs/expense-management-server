...
class ExpenseItemSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        return_object = {key: value for key, value in representation.items() if value is not None}
        return_object['presigned_url'] = self.get_presigned_url(instance)
        return return_object

class ExpenseReportSerializer(serializers.ModelSerializer):
    items = ExpenseItemSerializer(many=True, read_only=True)
    user = serializers.ReadOnlyField(source='user.username')
    report_number = serializers.CharField(required=False)
